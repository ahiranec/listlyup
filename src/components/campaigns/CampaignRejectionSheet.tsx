/**
 * Campaign Rejection Sheet
 * 
 * CANONICAL EXECUTOR for reject-campaign-request action
 * 
 * PURPOSE:
...
const REJECTION_REASONS = [
  { value: 'not-eligible', label: 'Does not meet campaign criteria' },
  { value: 'category-mismatch', label: 'Wrong category for campaign' },
  { value: 'quality-standards', label: 'Does not meet quality standards' },
  { value: 'duplicate', label: 'Duplicate listing in campaign' },
  { value: 'pricing', label: 'Pricing outside campaign range' },
  { value: 'location', label: 'Location not supported by campaign' },
  { value: 'other', label: 'Other reason (specify below)' },
] as const;

export function CampaignRejectionSheet({
  open,
  onOpenChange,
  listing,
  listingImage,
  ...
  const handleReject = async () => {
    ...
    toast.success('Campaign request rejected', {
      description: `${listing} has been rejected from "${campaign}"`,
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[90vh] sm:h-[85vh]">
        <SheetHeader className="mb-6">
          <SheetTitle>Reject Campaign Request</SheetTitle>
        </SheetHeader>
