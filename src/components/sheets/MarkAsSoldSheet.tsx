 ...
 *
 * Mobile: Bottom sheet with sticky header/footer and vh-based scroll
 * Desktop: Centered dialog with natural flow and max-height scroll
 */

import { useState } from 'react';
import { CheckCircle2, User, DollarSign, ChevronLeft, Star } from 'lucide-react';
...
export function MarkAsSoldSheet({
  ...
  return (
    <ResponsiveModal
      ...
      title="Mark as Sold"
      description={`Mark ${productTitle} as sold`}
      mobileHeight="h-[85vh]"
      desktopMaxWidth="max-w-lg"
      hideCloseButton={false}
    >
      {/* ── Header ── */}
      <div className="flex-none bg-background border-b z-10">
        {/* Mobile: back button + centered title */}
        <div className="flex items-center h-14 px-4 lg:hidden">
          <button
        ...
        {/* Desktop: simple title row (Dialog X handles close) */}
        <div className="hidden lg:flex items-center h-14 px-6">
          <h2 className="text-base font-semibold">Mark as Sold</h2>
        </div>

        {/* Product Preview — shared */}
        <div className="px-4 lg:px-6 pb-4 pt-1">
      ...
      {/* ── Scrollable Content ── */}
      {/* Mobile: viewport-based height | Desktop: flex-1 fills remaining dialog space */}
      <ScrollArea className="h-[calc(85vh-170px)] lg:flex-1 lg:h-auto lg:max-h-[50vh]">
      ...
      {/* ── Footer ── */}
      <div className="flex-none bg-background border-t p-4 lg:px-6">
        <Button
